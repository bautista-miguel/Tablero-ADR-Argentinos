...
def earnings_result_label(surprise):
    ...
    if abs(surprise) > 50:
        return "Dato a revisar"

    if surprise > 5:
        return "Sobre el consenso"

    if surprise < -5:
        return "Debajo del consenso"

    return "En línea con el consenso"


def earnings_result_icon(label):
    return {
        "Sobre el consenso": "🟢 Sobre el consenso",
        "En línea con el consenso": "⚪ En línea",
        "Debajo del consenso": "🔴 Debajo del consenso",
        "Dato a revisar": "⚠️ Dato a revisar",
        "Sin datos": "—",
    }.get(label, label)
...
with tab_fundamentals:
    ...
    st.caption(
        "Beta y EPS TTM provienen de Alpha Vantage. El P/E se recalcula "
        "con el precio actual. El próximo earnings surge del calendario de "
        "Alpha Vantage y el último reporte compara EPS reportado contra estimado."
    )
    ...
    if fundamentals.empty:
        ...
        # El P/E mostrado se calcula con precio actual / EPS TTM almacenado.
        # Así cambia junto con la cotización sin consumir llamadas adicionales.
        fundamentals_view["P/E calculado"] = None

        valid_dynamic_pe = (
            fundamentals_view["Precio hoy"].notna()
            & fundamentals_view["EPS TTM"].notna()
            & (fundamentals_view["EPS TTM"] > 0)
        )
        ...
        fundamentals_view.loc[
            ...
            fundamentals_view.loc[valid_dynamic_pe, "Precio hoy"]
            / fundamentals_view.loc[valid_dynamic_pe, "EPS TTM"]
        )

        fundamentals_view["P/E mostrado"] = fundamentals_view.apply(
            lambda row: (
                "N/M — EPS negativo"
                if pd.notna(row.get("EPS TTM")) and row.get("EPS TTM") <= 0
                else (
                    f"{row['P/E calculado']:.2f}"
                    if pd.notna(row.get("P/E calculado"))
                    else "—"
                )
            ),
            axis=1,
        ...
        valid_pe = (
            fundamentals_view["P/E calculado"].notna()
            & fundamentals_view["P/E sector"].notna()
            & (fundamentals_view["P/E calculado"] > 0)
        ...
        fundamentals_view.loc[
            ...
            fundamentals_view.loc[valid_pe, "P/E calculado"]
        ...
        d2.metric(
            ...
            (
                "N/M — EPS negativo"
                if pd.notna(detail.get("EPS TTM")) and detail.get("EPS TTM") <= 0
                else (
                    f"{detail['P/E calculado']:.2f}"
                    if pd.notna(detail.get("P/E calculado"))
                    else "—"
                )
            ),
        )
